...
def flatten_cnndm(example):
    # print(example)
    # summaries = [highlight.replace('\n', ' ') for highlight in example["highlights"]]
    return {
        "document": example["article"].replace('(CNN)', '').replace(' .', '.').replace('\r', ''),
...
if (dataset_name=='xsum'):
    ...
    train_dataset = train_data.map(flatten_cnndm, remove_columns=["highlights","article", "id"])
    test_dataset = test_data.map(flatten_cnndm, remove_columns=["highlights","article", "id"])
    valid_dataset = valid_data.map(flatten_cnndm, remove_columns=["highlights","article", "id"])
...
summ_model = BartForConditionalGeneration.from_pretrained(model_name)
...
documents = test_samples["document"]
...
article_batches = list(chunks(test_samples['document'], batch_size))
target_batches = list(chunks(test_samples['summary'], batch_size))

# print(article_batches[0])
...
for article_batch, target_batch in tqdm(zip(article_batches, target_batches), total=len(article_batches)):
    print("Processing: ", cur_num, " to ", cur_num+len(article_batch))
    cur_num += len(article_batch)
    ...
    result, results_ori = summ_model.generate(
        input_ids, 
        num_beams=num_beams, 
    ...
    dec = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in result]
    # print(dec)
    preds += dec
    srcs += article_batch
    tgts += target_batch
...
save_path = os.path.join(SUMM_RESULTS_DIR, "bart_xsum_large_dec")
...
with open(save_path + '.raw_src', "w+", encoding='utf-8') as out_file_src:
    for pa in srcs:
        out_file_src.write(str(''.join(pa.replace('\n', ' '))) + '\n')
...
for i, result_ori in enumerate(results_ori):
    ...
    for bm in range(num_beams):
        text_dec = tokenizer.decode(result_ori[bm][1], skip_special_tokens=True, clean_up_tokenization_spaces=False)
...
for i, ori_seq in enumerate(ori_seqs):
    # print("sys: ", ori_seq[0:3])
    # print("gold: ", summaries[0:3])
    scores = rouge.get_scores(ori_seq, summaries, avg=True)
    print(i, ". ASLI Rouge: ", scores)
    rouge_metric = datasets.load_metric('rouge')
    rouge_metric.add_batch(predictions=summaries, references=ori_seq)
    score_2 = rouge_metric.compute()
    print(i, ". datasets Rouge: ", score_2)
