...
class BaselineScorer:
    def __init__(self, model="questeval", do_weighter=False, load_cache=False):
        assert model in ["questeval", "feqa", "dae"], "Unrecognized baseline model"
        self.model = model
        self.do_weighter = do_weighter
        self.model_loaded = False
        self.cache = {}
        self.cache_file = HOME_DATA + "masters_fact_cache/cache_%s.json" % (self.model)
        if load_cache:
            self.load_cache()
    ...
    def load_model(self):
        if self.model == "questeval":
            ...
            sys.path.insert(0, HOME_REPO + "dae-factuality/")
            ...
            model_dir = HOME_REPO + "dae-factuality/models/dae_basic/"
        ...
        elif self.model == "summac":
            sys.path.insert(0, HOME_REPO + "summac/")
            from model_summac import SummaCZS

            self.scorer = SummaCZS(granularity="sentence", model_name="vitc")
    ...
    def score_summac(self, documents, generateds, **kwargs):
        score = self.scorer.score([document], [summary])
