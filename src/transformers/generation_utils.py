...
from .generation_beam_search import BeamScorer, BeamSearchScorer, ConstrainedBeamSearchScorer, FactBeamSearchScorer
...
class GenerationMixin:
    ...
    @staticmethod
    def _update_model_kwargs_for_generation_expand(
        outputs: ModelOutput, model_kwargs: Dict[str, Any], is_encoder_decoder: bool = False
    ) -> Dict[str, Any]:
        # print("[DEBUG] outputs in kwargs update: ", str(outputs))
        # update past
        if "past_key_values" in outputs:
            # print("DEBUG past keys: ", outputs.past_key_values)
            model_kwargs["past"] = outputs.past_key_values
        elif "mems" in outputs:
            model_kwargs["past"] = outputs.mems
        elif "past_buckets_states" in outputs:
    ...
    def generate_beam_expansion(
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            beam_params = {}
            beam_params["input_ids"] = input_ids
            beam_params["beam_scorer"] = beam_scorer
            beam_params["logits_processor"] = logits_processor
            beam_params["stopping_criteria"] = stopping_criteria
            beam_params["pad_token_id"] = pad_token_id
            beam_params["eos_token_id"] = eos_token_id
            beam_params["output_scores"] = output_scores
            beam_params["return_dict_in_generate"] = return_dict_in_generate
            beam_params["synced_gpus"] = synced_gpus

            params, model_kwargs = self.init_beam_search_expand_single(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
    ...
    def init_beam_search_expand_single(
        ...
        **model_kwargs,
        ...
        # init values
        params["logits_processor"] = logits_processor if logits_processor is not None else LogitsProcessorList()
        params["stopping_criteria"] = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
            warnings.warn(
                "`max_length` is deprecated in this function, use"
                " `stopping_criteria=StoppingCriteriaList(MaxLengthCriteria(max_length=max_length))` instead.",
            ...
            params["stopping_criteria"] = validate_stopping_criteria(stopping_criteria, max_length)
        if len(stopping_criteria) == 0:
            warnings.warn("You don't have defined any stopping_criteria, this will likely loop forever", UserWarning)
        params["pad_token_id"] = pad_token_id if pad_token_id is not None else self.config.pad_token_id
        params["eos_token_id"] = eos_token_id if eos_token_id is not None else self.config.eos_token_id
        params["output_scores"] = output_scores if output_scores is not None else self.config.output_scores
        params["output_attentions"] = output_attentions if output_attentions is not None else self.config.output_attentions
        ...
        # init attention / hidden states / scores tuples
        params["scores"] = () if (return_dict_in_generate and output_scores) else None
        params["beam_indices"] = (
            tuple(() for _ in range(batch_beam_size)) if (return_dict_in_generate and output_scores) else None
        )
        params["decoder_attentions"] = () if (return_dict_in_generate and output_attentions) else None
        params["cross_attentions"] = () if (return_dict_in_generate and output_attentions) else None
        params["decoder_hidden_states"] = () if (return_dict_in_generate and output_hidden_states) else None
        ...
        params["beam_scores"] = torch.zeros((params["batch_size"], params["num_beams"]), dtype=torch.float, device=input_ids.device)
        params["beam_scores"][:, 1:] = -1e9
        params["beam_scores"] = params["beam_scores"].view((params["batch_size"] * params["num_beams"],))

        params["this_peer_finished"] = False  # used by synced_gpus only
    ...
    def beam_search_expand_single(
        ...
        **model_kwargs,
        ...
        beams = []
        beam_scores = []
        for lp, path, beam_score in filled_paths:
            beams.append(path.unsqueeze(0))
            beam_scores.append(beam_score.unsqueeze(0))
        input_path = torch.cat(beams, dim=0)

        params["beam_scores"] = torch.cat(beam_scores, dim=0)
        
        # TODO FT order or modify the beam indices
        if synced_gpus:
            # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            # The following logic allows an early break if all peers finished generating their sequence
        ...
        # TODO FT fix after removing loop?
        outputs = self(
            **model_inputs,
            return_dict=True,
        ...
        # print("[EXPFT] BEFORE params['beam_scores']: ", params["beam_scores"])
        ...
        if return_dict_in_generate:
            ...
            if params["output_attentions"]:
                params["decoder_attentions"] += (
                    (outputs.decoder_attentions,) if self.config.is_encoder_decoder else (outputs.attentions,)
                )
                if self.config.is_encoder_decoder:
                    cross_attentions += (outputs.cross_attentions,)

            if params["output_hidden_states"]:
                params["decoder_hidden_states"] += (
                    (outputs.decoder_hidden_states,)
                    if self.config.is_encoder_decoder
                    else (outputs.hidden_states,)
                )
        ...
        params["beam_scores"] = beam_outputs["next_beam_scores"]
        beam_next_tokens = beam_outputs["next_beam_tokens"]
        beam_idx = beam_outputs["next_beam_indices"]

        input_path = torch.cat([input_path[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)

        model_kwargs = self._update_model_kwargs_for_generation_expand(
            outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
        )
        if model_kwargs["past"] is not None:
        ...
        # print("EXPFT params['indices'] ", params['beam_indices'])
        # print("[EXPFT] AFTER params['beam_scores']: ", params["beam_scores"])
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return BeamSearchEncoderDecoderOutput(
                    ...
                    beam_indices=sequence_outputs["beam_indices"],
                    encoder_attentions=params["encoder_attentions"],
                    encoder_hidden_states=params["encoder_hidden_states"],
                    decoder_attentions=params["decoder_attentions"],
                    cross_attentions=params["cross_attentions"],
                    decoder_hidden_states=params["decoder_hidden_states"],
                )
            else:
                return BeamSearchDecoderOnlyOutput(
                    sequences=sequence_outputs["sequences"],
                    ...
                    scores=params["scores"],
                    beam_indices=sequence_outputs["beam_indices"],
                    attentions=params["decoder_attentions"],
                    hidden_states=params["decoder_hidden_states"],
                )
        else:
            return sequence_outputs["sequences"]
